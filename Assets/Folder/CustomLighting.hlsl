...
void AdditionalLight_float(float3 WorldPos, int lightID, out float3 Direction, out float3 Color, out float Attenuation)
{
    ...
    int safeID = clamp(lightID, 0, max(0, lightCount - 1));
    Light light = GetAdditionalLight(safeID, WorldPos);

    bool isValid = (lightID >= 0) && (lightID < lightCount);

    Direction = light.direction;
    Color = light.color;
    Attenuation = isValid ? light.distanceAttenuation : 0.0;
#endif
}
...
#ifndef SHADERGRAPH_PREVIEW
    int lightCount = GetAdditionalLightsCount();
    float isValid = step(0.0, (float) lightID) * step((float) lightID, (float) (lightCount - 1));

    Light light = GetAdditionalLight(lightID, WorldPos);

    Direction = lerp(Direction, light.direction, isValid);
    Color = lerp(Color, light.color, isValid);
    Attenuation = lerp(Attenuation, light.distanceAttenuation, isValid);
#endif
